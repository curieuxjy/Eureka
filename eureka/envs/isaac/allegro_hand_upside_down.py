...
class AllegroHandUpsideDown(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        if self.obs_type == "full_state" or self.asymmetric_obs:
            ...
            self.dof_force_tensor = gymtorch.wrap_tensor(dof_force_tensor).view(self.num_envs, self.num_allegro_hand_dofs)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        allegro_hand_asset_file = os.path.normpath("mjcf/open_ai_assets/hand/allegro_hand.xml")

        if "asset" in self.cfg["env"]:
            allegro_hand_asset_file = os.path.normpath(self.cfg["env"]["asset"].get("assetFileName", allegro_hand_asset_file))
        ...
        allegro_hand_start_pose.r.x = 0
        allegro_hand_start_pose.r.y = 1
        allegro_hand_start_pose.r.z = 0
        allegro_hand_start_pose.r.w = 0
        ...
        allegro_hand_rb_count = self.gym.get_asset_rigid_body_count(allegro_hand_asset)
    ...
    def compute_full_state(self, asymm_obs=False):
        if asymm_obs:
            ...
            self.states_buf[:, 2*self.num_allegro_hand_dofs:3*self.num_allegro_hand_dofs] = self.force_torque_obs_scale * self.dof_force_tensor

            obj_obs_start = 3*self.num_allegro_hand_dofs  # 72
            ...
            self.obs_buf[:, 0:self.num_allegro_hand_dofs] = unscale(self.allegro_hand_dof_pos,
                                                                   self.allegro_hand_dof_lower_limits, self.allegro_hand_dof_upper_limits)
            self.obs_buf[:, self.num_allegro_hand_dofs:2*self.num_allegro_hand_dofs] = self.vel_obs_scale * self.allegro_hand_dof_vel
            self.obs_buf[:, 2*self.num_allegro_hand_dofs:3*self.num_allegro_hand_dofs] = self.force_torque_obs_scale * self.dof_force_tensor
    ...
    def reset_idx(self, env_ids, goal_env_ids):
        ...
        pos = self.allegro_hand_dof_default_pos + self.reset_dof_pos_noise * rand_delta

...
class AllegroHandSwingCup(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.dof_force_tensor = gymtorch.wrap_tensor(dof_force_tensor).view(self.num_envs, self.num_allegro_hand_dofs * 2 + self.num_object_dofs * 2)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            another_hand_shape_props = self.gym.get_actor_rigid_shape_properties(env_ptr, allegro_hand_another_actor)
    ...
    def reset_idx(self, env_ids, goal_env_ids):
        ...
        self.prev_targets[env_ids, self.num_allegro_hand_dofs*2:self.num_allegro_hand_dofs*2 + 1] = to_torch([0], device=self.device)
        self.cur_targets[env_ids, self.num_allegro_hand_dofs*2:self.num_allegro_hand_dofs*2 + 1] = to_torch([0], device=self.device)
        self.prev_targets[env_ids, self.num_allegro_hand_dofs*2 + 1:self.num_allegro_hand_dofs*2 + 2*1] = to_torch([0], device=self.device)
        self.cur_targets[env_ids, self.num_allegro_hand_dofs*2 + 1:self.num_allegro_hand_dofs*2 + 2*1] = to_torch([0], device=self.device)

...
class AllegroHandCatchOver2Underarm(VecTask):
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_relative_control:
            targets = self.prev_targets[:, self.actuated_dof_indices] + self.allegro_dof_speed_scale * self.dt * self.actions[:, 6:22]
            ...
            targets = self.prev_targets[:, self.actuated_dof_indices + 16] + self.allegro_dof_speed_scale * self.dt * self.actions[:, 22:38]
            self.cur_targets[:, self.actuated_dof_indices + 16] = tensor_clamp(targets,
                                                                          self.allegro_hand_dof_lower_limits[self.actuated_dof_indices], self.allegro_hand_dof_upper_limits[self.actuated_dof_indices])
